...
pub struct BatchItemResult {
    pub index: u32,
    pub success: bool,
    /// Non-zero on failure; maps to ` RustAcademyError` discriminant.
    pub error_code: u32,
}
...
pub fn batch_create(
...
) -> Result<Vec<BatchItemResult>,  RustAcademyError> {
    caller.require_auth();
    ...
    for (i, item) in items.iter().enumerate() {
        ...
        if item.amount <= 0 {
            results.push_back(BatchItemResult { index: idx, success: false, error_code:  RustAcademyError::InvalidAmount as u32 });
            continue;
        }

        if get_escrow(env, &item.escrow_id).is_some() {
            results.push_back(BatchItemResult { index: idx, success: false, error_code:  RustAcademyError::EscrowExists as u32 });
            continue;
        }
...
pub fn batch_release(
    ...
    for (i, id) in escrow_ids.iter().enumerate() {
        ...
        let mut entry = match get_escrow(env, &id) {
            Some(e) => e,
            None => {
                results.push_back(BatchItemResult { index: idx, success: false, error_code:  RustAcademyError::EscrowNotFound as u32 });
                continue;
            }
        };

        if entry.status != EscrowStatus::Pending {
            results.push_back(BatchItemResult { index: idx, success: false, error_code:  RustAcademyError::AlreadySpent as u32 });
            continue;
        }

        if entry.expires_at > 0 && now >= entry.expires_at {
            results.push_back(BatchItemResult { index: idx, success: false, error_code:  RustAcademyError::EscrowExpired as u32 });
            continue;
        }
...
pub fn batch_refund(
    ...
    for (i, id) in escrow_ids.iter().enumerate() {
        ...
        if entry.expires_at == 0 || now < entry.expires_at {
            results.push_back(BatchItemResult { index: idx, success: false, error_code:  RustAcademyError::EscrowNotExpired as u32 });
            continue;
        }
